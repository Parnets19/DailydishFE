...
const navigate = useNavigate()
  ...
  const subtotal = useMemo(() => {
    return item?.allProduct?.reduce((acc, item) => {
      return Number(acc) + Number(item.quantity) * Number(item.foodItemId.totalprice);
    }, 0);
  }, [item]);

  const total = useMemo(() => {
  return Number(item?.subTotal)+Number(item?.Cutlery)+Number(item?.tax)+Number(item?.delivarytype)
  }, [subtotal,item]);

  return (
    <>
      <MdArrowBackIosNew onClick={()=>navigate('/dashboard')} style={{color:'black',fontSize:'26px',marginLeft:'5px',marginTop:'5px'}}/>

    <div style={{ backgroundColor: "white", padding: "5%" }}>
      <div id="pdf" className="pl-1 pr-1">
        <div style={{ textAlign: "center",fontSize:'30px',fontWeight:'700',fontFamily:"Imperial Script"}}>Daily Dish</div>
        <div style={{ textAlign: "center",fontSize:'12px' }}>Banglore</div>
        <div style={{ textAlign: "center",fontSize:'12px' }}>GSTIN : 29AARFC3236D1ZF</div>
    
        <span className="mt-3">
          Type:{item?.orderdelivarytype}
        </span>
        <br />
        ...
        <Row>
          <Col md={"12"}>
            ...
            <Table>
                    ...
                    <tbody>
                    <tr>
                        <td style={{ textAlign: "left",fontSize:'14px',fontWeight:"400"  }}>Subtotal</td>
                        <td style={{ textAlign: "right",fontSize:'14px',fontWeight:"400"  }}> ₹ {item?.subTotal}</td>
                      </tr>
                      {item?.delivarytype <= 0 ? (
                        <tr>
                          <td style={{ textAlign: "left",fontSize:'14px',fontWeight:"400" }}>
                            Delivery Charge
                          </td>
                          <td style={{ textAlign: "right",fontSize:'14px',fontWeight:"400"}}> Free</td>
                        </tr>
                      ) : (
                        <tr>
                          <td style={{ textAlign: "left",fontSize:'14px',fontWeight:"400" }}>
                      ...
                      <tr>
                        <td style={{ textAlign: "left",fontSize:'14px',fontWeight:"400"  }}>Tax (5%)</td>
                        <td style={{ textAlign: "right",fontSize:'14px',fontWeight:"400"  }}> ₹ {item?.tax}</td>
                      </tr>
                      {item?.Cutlery > 0 ? (
                        <>
                          <tr>
                            <td style={{textAlign: "left",fontSize:'14px',fontWeight:"400"  }}>Cutley</td>
                            <td style={{ textAlign: "right",fontSize:'14px',fontWeight:"400"  }}>
                              {" "}
                              ₹ {item?.Cutlery}
                            </td>
